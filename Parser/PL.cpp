#include <iostream>
#include <string>
#include <fstream>
#include <stack>
#include <algorithm>

using namespace std;


	char homepage_indicator = 'N';
	std::stack<std::string>	stk_subject, stk_predicate, stk_object;
	std::ofstream ofs;

	std::string get_subject(string& triple);
	std::string get_predicate(string& triple);
	std::string get_object(string& triple);


//***************************************************
//		Struct to facilitate sorting!
//
//***************************************************
struct stc_Triple
{
    string subject;
    string predicate;
    string object;


    //stc_Triple(const string s, const string p, const string p):
      //  subject(s),predicate(p),object(o){}
};

bool SortByPredicate(const stc_Triple & p1, const stc_Triple & p2)
{
    return p1.subject < p2.subject || (p1.subject == p2.subject && p1.predicate < p2.predicate) || (p1.subject == p2.subject && p1.predicate == p2.predicate && p1.object < p2.object) ;
}

bool SortByObject(const stc_Triple & p1, const stc_Triple & p2)
{
    return p1.object < p2.object;
}

	string gsubject, predicate, object, tmp_s;
	unsigned long number_of_triples = 0, counter_of_structs = 0;




//**********************************************************
void add_json_ld_doc_pre(std::stack<std::string>& stk_subject, std::stack<std::string>& stk_predicate, std::stack<std::string>& stk_object);
//**********************************************************
void add_json_iswc_pre(std::stack<std::string>& stk_subject, std::stack<std::string>& stk_predicate, std::stack<std::string>& stk_object);
//**********************************************************
void add_json_3triples_sub(std::stack<std::string>& stk_subject, std::stack<std::string>& stk_predicate, std::stack<std::string>& stk_object);
//**********************************************************

stc_Triple * triples_structure;

//******************************************************************************
int main()
{
	//string subject, predicate, object, tmp_s;
	std::string  triple;

	
	std::ifstream in("./dataset/KG.nt");
	ofs.open ("./output/PL.json", std::ofstream::out | std::ofstream::app);
	

	while(getline(in, tmp_s,'.'))
	{
		number_of_triples ++;
	}
	in.close();	

	triples_structure = new stc_Triple[number_of_triples];

		

	in.open("./dataset/KG.nt");
	
	while (std::getline(in, triple))
	{

		gsubject   = get_subject(triple);
		predicate = get_predicate(triple);
		if ( predicate == "\"homepage\"")
			homepage_indicator = 'Y';
		object    = get_object(triple);

		triples_structure[counter_of_structs].subject   = predicate;
		triples_structure[counter_of_structs].predicate = gsubject;
		triples_structure[counter_of_structs].object   = object;
		counter_of_structs++;

	}
	in.close();
	sort(triples_structure, triples_structure + counter_of_structs, SortByPredicate);

	
	ofs << "[\n";
	
	for (unsigned long i=0; i < counter_of_structs; i++)
	{
	gsubject   = triples_structure[i].subject;  //Switching between subject and predicate
	predicate = triples_structure[i].predicate;
	object    = triples_structure[i].object;
	
//cout << i << "->" << subject << " " << predicate << "  " << object << "\n";
///*
	if (stk_subject.empty())
		stk_subject.push(gsubject);

	

	
	else if ( stk_subject.top() != gsubject)
	{
		//add_json_ld_doc_pre(stk_subject,stk_predicate,stk_object);
		add_json_iswc_pre(stk_subject,stk_predicate,stk_object);
		//add_json_3triples_sub(stk_subject,stk_predicate,stk_object);
		stk_subject.push(gsubject);
		
	}

	stk_predicate.push(predicate);
	stk_object.push(object);

	}




	if (!stk_subject.empty())				
		//add_json_ld_doc_pre(stk_subject,stk_predicate,stk_object);
		//add_json_3triples_sub(stk_subject,stk_predicate,stk_object);
		add_json_iswc_pre(stk_subject,stk_predicate,stk_object);
		

		ofs << "\n";
		ofs << "{}]";
		ofs.close();
//*/

return 0;
}// int main()

//******************************************************
//
//******************************************************
void add_json_3triples_sub(std::stack<std::string>& stk_subject, std::stack<std::string>& stk_predicate, std::stack<std::string>& stk_object){



	while ( stk_predicate.size() > 0)
	{
			
		ofs << "{\n";
		ofs << "\t\"_id\":" << stk_subject.top() << ",\n";
		ofs << "\t\"Predicate \":" << stk_predicate.top() << "," ;
		ofs << "\t\"Object \": "   << stk_object.top() ;
		ofs << "},\n";
		stk_predicate.pop();
		stk_object.pop();

	}
	
	stk_subject.pop();


	

}//add_json_3triples_sub

//******************************************************
//
//******************************************************
void add_json_iswc_pre(std::stack<std::string>& stk_subject, std::stack<std::string>& stk_predicate, std::stack<std::string>& stk_object){

	
	ofs << "{\n";
	ofs << "\t\"_id\":" << stk_subject.top() << ",\n";
	stk_subject.pop();

	ofs << "\t\"Subjects\": [" ;

	while ( stk_predicate.size() > 1)
	{
		ofs << stk_predicate.top() << ",";
		stk_predicate.pop();

	}
		ofs << stk_predicate.top() << "],";
		stk_predicate.pop();

	ofs << "\t\"Objects\": [" ;

	while ( stk_object.size() > 1)
	{
		ofs << stk_object.top() << ",";
		stk_object.pop();

	}
		ofs << stk_object.top() << "]";
		stk_object.pop();

	ofs << "},\n";




}//add_json_iswc_sub

//******************************************************
//
//******************************************************
void add_json_ld_doc_pre(std::stack<std::string>& stk_subject, std::stack<std::string>& stk_predicate, std::stack<std::string>& stk_object){
	//std::ofstream ofs;
	//ofs.open ("./ISWC/iswc.json", std::ofstream::out | std::ofstream::app);
	std::string temp_top_predicate;
	ofs << "{\n";
	ofs << "\t\"_id\":" << stk_subject.top() << ",\n";
	stk_subject.pop();


	while (stk_object.size() > 1 )
	{
		//if (stk_predicate.size() > 1)
		//{				
		temp_top_predicate = stk_predicate.top(); 
		stk_predicate.pop();
		//}
//cout << temp_top_predicate << "\n";
		if ( stk_predicate.top() != temp_top_predicate)
		{
			ofs << "\t" << temp_top_predicate << ":" << stk_object.top() << ",\n";
			stk_object.pop();
		}else
		{
			
			ofs << "\t" << temp_top_predicate << ":[";
			ofs <<  stk_object.top() << ",";
	 		stk_object.pop();
			//temp_top_predicate = stk_predicate.top();
			//stk_predicate.pop();
			
			while (stk_predicate.top() == temp_top_predicate)
			{
				stk_predicate.pop();

				if (!stk_predicate.empty())				
				{
					if (stk_predicate.top() == temp_top_predicate)
						ofs <<  stk_object.top() << ",";
					else
						ofs <<  stk_object.top() << "]";
				}else
					ofs <<  stk_object.top() << "]";
		 		stk_object.pop();
				//temp_top_predicate = stk_predicate.top();
				//stk_predicate.pop();
				if (stk_object.empty())
					break;
			}

		if (!stk_object.empty())
			ofs <<  ",\n";
			//stk_predicate.push(temp_top_predicate);
			//ofs <<  stk_object.top() << "],\n";
	 		//stk_object.pop();
			//ofs <<  stk_object.top() << "],\n";

		}
	}
	
	if (!stk_predicate.empty())
	{
		ofs << "\t" << stk_predicate.top() << ":" << stk_object.top() << "\n";
		stk_object.pop();
		stk_predicate.pop();
	}
		ofs << "},\n";


}//add_json_doc



//******************************************************
//
//******************************************************
std::string get_subject(string& triple ){



	unsigned first, last;
	string lsubject;

	
		first = 0;
		last = triple.find(">");
		lsubject = triple.substr (0,last);
		triple = triple.substr (last+1,triple.length());
		while (first < lsubject.length())
		{
			first = lsubject.find("/");
			lsubject = lsubject.substr (first+1, lsubject.length());
			//cout << subject << endl;
		}

		lsubject = "\"" + lsubject + "\"";
		//cout << subject << endl;
return lsubject;
}

//******************************************************
//
//******************************************************
std::string get_predicate(string& triple ){



	unsigned first, last;
	string predicate;

		first = 0;
		last = triple.find(">");
		predicate = triple.substr (0,last);
		triple = triple.substr (last+1,triple.length());
		while (first <= predicate.length())
		{
			first = predicate.find("/");
			predicate = predicate.substr (first+1, predicate.length());
			//cout << subject << endl;
		}


		if ( predicate.find("#") < predicate.length() )
			predicate = predicate.substr ((predicate.find("#"))+1, predicate.length());

		predicate = "\"" + predicate + "\"";
		//cout << predicate << endl;

	
return predicate;
}
















//******************************************************
//
//******************************************************



bool is_number(string str){

	for(size_t i=0; i < str.length() ; i++)
	{
		if (!isdigit( str[ i ] ))
			return false;
	}

return true;
}//is_number










//******************************************************
//
//******************************************************
std::string get_object(string& triple){


	unsigned first, last;
	string object;


		first = 0;
		

		if (homepage_indicator == 'Y')
		{
			first = triple.find("<");
			last = triple.find(">");
			object = triple.substr (first+1, last-3);
			object = "\"" + object + "\"";
			homepage_indicator = 'N';

		}else if ( (triple.find(">") < triple.length())  && (triple.find("^") > triple.length()) )
		{
			object = triple.substr (0,triple.find(">"));
			if (object.substr(object.length()-2,object.length()).find("/") <= object.length())
				object = triple.substr (0,triple.find(">")-1);


			while (first < object.length())
			{
				first = object.find("/");
				object = object.substr (first+1, object.length());
				//cout << subject << endl;
			}
				if ( object.find("#") <= object.length())
				{
					first = object.find("#");
					object = object.substr (first+1, object.length());
				}

				object = "\"" + object + "\"";
		}
		else if (triple.find(">") < triple.length()  && (triple.find("^") < triple.length()) )
		{
			first = triple.find("\"");			
			last  = triple.substr (first+1,triple.length()).find("\"");



			if (is_number(triple.substr (first+1,last)) || triple.substr (first+1,last).find(".") <= triple.substr (first+1,last).length() ) 
			{
				object = triple.substr (first+1,last);
				//cout << "number-->" << object << "\n";

			}else if ( triple.substr (first+1,last).find("-") <= triple.substr (first+1,last).length() )  
			{
				object = triple.substr (first+1,last);

		

					if (object.find("T") <= object.length())
						object = object.substr (0,object.find("T"));// + "\"";

					object = "{$date:\"" + object + "T00:00:00.000Z\"}";  //ISODate("2013-10-01T00:00:00.000Z"),
			
				
			
				/*
				if ( object.find("-") <= object.length())  //commented for numeric dates
				{
			
				//object = "{$date:" + object + "}";  //ISODate("2013-10-01T00:00:00.000Z"),

				//object = "ISODate(\"" + object + "T00:00:00.000Z\")";  //ISODate("2013-10-01T00:00:00.000Z"),
					object = "{$date:" + object + "T00:00:00Z}";  //ISODate("2013-10-01T00:00:00.000Z"),
				//object = "ISODate(\"" + object + "\")";  //ISODate("2013-10-01T00:00:00.000Z"),
				
				}

				*/
			}else
			{
				object = triple.substr (first,last+2);			
			}

		
			

					//object = "\"" + object + "\"";  //"attDate":{$date:"2013-11-20T23:32:18Z"}
			


			

		}else
		{
			first = triple.find("\"");
			object = triple.substr (first,triple.length()-2);
			if ( object.find("@") <= object.length())
			{
				object = triple.substr (first,triple.find("@")-1);
				
				//stk_predicate.push("\"language\"");
				string tmp = triple.substr(triple.find("@")+1,2);
				tmp = "\""+ tmp +"\"";		
				//stk_object.push(tmp);



				triples_structure[counter_of_structs].subject   = "\"language\"";
				triples_structure[counter_of_structs].predicate = gsubject;
				triples_structure[counter_of_structs].object   = tmp;
				counter_of_structs++;



			
			}

		}

		//cout << object << endl;


return object;
}


















/*
//******************************************************
//
//******************************************************
std::string get_object(string& triple){


	unsigned first, last;
	string object;


		first = 0;
		

		if (homepage_indicator == 'Y')
		{
			first = triple.find("<");
			last = triple.find(">");
			object = triple.substr (first+1, last-3);
			object = "\"" + object + "\"";
			homepage_indicator = 'N';

		}else if ( (triple.find(">") < triple.length())  && (triple.find("^") > triple.length()) )
		{
			object = triple.substr (0,triple.find(">"));
			if (object.substr(object.length()-2,object.length()).find("/") <= object.length())
				object = triple.substr (0,triple.find(">")-1);


			while (first < object.length())
			{
				first = object.find("/");
				object = object.substr (first+1, object.length());
				//cout << subject << endl;
			}
				if ( object.find("#") <= object.length())
				{
					first = object.find("#");
					object = object.substr (first+1, object.length());
				}

				object = "\"" + object + "\"";
		}
		else if (triple.find(">") < triple.length()  && (triple.find("^") < triple.length()) )
		{
			first = triple.find("\"");			
			last  = triple.substr (first+1,triple.length()).find("\"");
			object = triple.substr (first,last+2);
			if ( object.find("T") <= object.length())
				object = object.substr (0,object.find("T")) + "\"";
			

		}else
		{
			first = triple.find("\"");
			object = triple.substr (first,triple.length()-2);
			if ( object.find("@") <= object.length())
			{
				object = triple.substr (first,triple.find("@")-1);
				
				//stk_predicate.push("\"language\"");
				string tmp = triple.substr(triple.find("@")+1,2);
				tmp = "\""+ tmp +"\"";		
				//stk_object.push(tmp);



				triples_structure[counter_of_structs].subject   = "\"language\"";
				triples_structure[counter_of_structs].predicate = gsubject;
				triples_structure[counter_of_structs].object   = tmp;
				counter_of_structs++;



			
			}

		}

		//cout << object << endl;


return object;
}



*/
